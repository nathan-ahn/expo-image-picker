...
internal struct MediaHandler {
  ...
  internal func handleMultipleMedia(_ selection: [PHPickerResult]) async throws -> [AssetInfo] {
    ...
    return try await concurrentMapEnumeratedQueued(selection) { (selectedItem, index) in
      ...
      if options.mediaTypes.contains(.livePhotos) && (itemProvider.hasItemConformingToTypeIdentifier(UTType.livePhoto.identifier) || itemProvider.hasItemConformingToTypeIdentifier("com.apple.live-photo-bundle")) {
        maybeAssetInfo = try await handleLivePhoto(from: selectedItem)
      } else if itemProvider.canLoadObject(ofClass: PHLivePhoto.self) && options.mediaTypes.contains(.livePhotos) {
        maybeAssetInfo = try await handleLivePhoto(from: selectedItem)
  ...
  private func handleImage(from selectedImage: PHPickerResult) async throws -> AssetInfo {
    ...
    if options.fastCopy {
      ...
      return AssetInfo(assetId: selectedImage.assetIdentifier,
                       type: "image",
                       uri: targetUrl.absoluteString
      )
    }
